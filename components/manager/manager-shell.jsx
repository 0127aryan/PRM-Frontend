...
import { ManagerTopBar } from '@/components/manager/manager-top-bar';
import { useCurrentUser } from '@/hooks/useCurrentUser';

/**
 * @param {{ children: React.ReactNode, title: string, subtitle?: string }} props
 */
export function ManagerShell({ children, title, subtitle }) {
  const { user, loading } = useCurrentUser({ requiredRole: 'MANAGER' });
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-8">
        <p className="max-w-md text-center text-sm text-slate-600">
          Could not verify your manager session. Sign in again at{' '}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="ml-[240px] flex min-h-screen flex-col">
        <ManagerTopBar title={title} subtitle={subtitle} user={user} />
